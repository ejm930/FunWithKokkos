...
struct my_pipeline {
    ...
    void dispatch() {
        ...
        auto const &vect_ = vect; // this aliasing is good


        auto const &vect_ = vect; // shallow shallow copy
        auto const vect_ = vect; // shallow copy
        auto const vect_ = Kokkos::Deep_Coyo(vect, vect_); // deep copy

        auto const &factor_ = factor; // eric
        float _factor = factor; // bob

        Kokkos::parallel_for("my_pipeline::vect_fill",
        Kokkos::RangePolicy<Space>(0, N),
        KOKKOS_LAMBDA(int i) {
            vect_(i) = multiply((float)i, factor_);
        });

        Multiply_Op func_1(vect);
        Kokkos::parallel_for("my_pipeline::vect_fill",
            Kokkos::RangePolicy<Space>(0, N),
            func_1
        );
    ...
    KOKKOS_INLINE_FUNCTION
        float multiply(float i, float factor, eric_t & vect) {
            return factor*i;
        }
};
...
class Multiply_Op {
    public:
        //Multiply(size_t _N) : vect(){}

        Multiply( view_t& _vect, float _factor) : vect(_vect), factor(_factor) { } 

        KOKKOS_INLINE_FUNCTION 
        float operator() (const int i) const {
             vect_(i) = multiply((float)i * factor);
             this->vect_(i) = multiply((float)i * factor);
        }
        void dispathc() {
    ...
    private: 
        float factor;
        Kokkos::View<float *, Kokkos::DefaultExecutionSpace>& vect;
}
